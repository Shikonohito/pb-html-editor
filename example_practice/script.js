...
def run_test_case(code_str, inputs_list):
    ...
    return "".join(runner.outputs)
                ...
                const section = checkCodeBtn.closest('.lesson-section');
                if (section) {
                    const sectionId = section.id;
                    const icon = document.querySelector(`.task-icon[data-target="${sectionId}"]`);
                    if (icon) {
                        icon.classList.remove('success', 'fail');
                        if (allPassed) {
                            icon.classList.add('success');
                            icon.innerHTML = '<svg width="19" height="19"><use href="#icon-check"></use></svg>';
                        } else {
                            icon.classList.add('fail');
                            icon.innerHTML = '<svg width="24" height="24"><use href="#icon-cross"></use></svg>';
                        }
                    }
                }
            }
